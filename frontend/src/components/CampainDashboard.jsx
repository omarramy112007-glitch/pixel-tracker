...
export default function CampaignDashboard({ campaignId }) {
  ...
  return (
    <div style={{ padding: "1rem", fontFamily: "Arial, sans-serif" }}>
      ...
      <p><strong>SMS Sent:</strong> {campaign.sms_sent}</p>
      ...
      <p><strong>Sent:</strong> {funnel.total_sent}</p>
      <p>
        <strong>Replied:</strong> {funnel.replied} 
        ({funnel.drop_off_to_reply_pct}% drop-off)
      </p>
      <p>
        <strong>Converted:</strong> {funnel.converted} 
        ({funnel.drop_off_to_conversion_pct}% drop-off)
      </p>

      <div style={{ display: "flex", gap: "10px", marginTop: "10px" }}>
        <div style={{ flex: 1, background: "#ddd", padding: "10px", textAlign: "center" }}>
          Sent<br />{funnel.total_sent}
        </div>
        <div style={{ flex: 1, background: "#ffc107", padding: "10px", textAlign: "center" }}>
          Replied<br />{funnel.replied}
        </div>
        <div style={{ flex: 1, background: "#28a745", padding: "10px", textAlign: "center" }}>
          Converted<br />{funnel.converted}
        </div>
      </div>
