...
async def on_startup() -> None:
    if GMAIL_WATCH_MODE == "watch":
        try:
            from outreach_engine.tracking.gmail_watcher import start_watch as _start_watch
            result = _start_watch()
...
def _start_poll_task() -> None:
    try:
        from outreach_engine.tracking.gmail_watcher import start_reply_polling as _start_reply_polling
        log(f"👂 Starting reply polling every {GMAIL_POLL_INTERVAL}s", force=True)
        asyncio.create_task(_start_reply_polling(GMAIL_POLL_INTERVAL))
    except Exception as e:
...
async def check_replies_get():
    ...
    """
    try:
        from outreach_engine.tracking.gmail_watcher import check_for_replies as _check_for_replies
        processed = _check_for_replies()
