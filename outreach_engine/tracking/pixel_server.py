...
# IMPORTANT:
# Expose router so imports do not fail:
# from outreach_engine.tracking.pixel_server import router
router = APIRouter()
...
try:
    ...
    # Try optional router import
    try:
        from outreach_engine.tracking.gmail_watcher import router as gmail_router
        ...
        gmail_router = None
        log(
            f"⚠ Gmail router missing — continuing without router: {router_error}",
            force=True,
        )
...
async def on_startup() -> None:
    """
    Start Gmail watcher/poller safely.
    """
    ...
    if GMAIL_WATCH_MODE == "watch":
        if start_watch:
            try:
                result = start_watch()
                log(
                    f"✅ Gmail watch renewed on startup: {result}",
                    force=True,
                )
                return
                ...
                log(
                    f"⚠ Gmail watch renewal failed: {e}",
                    force=True,
                )
...
def _start_poll_task() -> None:
    """
    Safe background polling startup.
    """
    ...
    try:
        log(
            f"👂 Starting reply polling every {GMAIL_POLL_INTERVAL}s",
            force=True,
        )

        asyncio.create_task(
            start_reply_polling(GMAIL_POLL_INTERVAL)
        )
...
def _resolve_system_lead_id_from_email(
    email: Optional[str],
) -> Optional[str]:
    if not email:
        return None
...
def _update_outreach_leads(
    ...
    try:
        res = (
            supabase.table("outreach_leads")
            .select(
                "open_count, click_count, email_opened, link_clicked"
            )
            .eq("id", lead_id)
            .limit(1)
            .execute()
...
async def _track_open_db(
    ...
    try:
        ...
        log(
            f"📬 OPEN TRACKED → Lead {lead_id} | count={current_count + 1}",
            force=True,
        )
...
async def _track_click_db(
    ...
    try:
        ...
        log(
            f"🖱 CLICK TRACKED → Lead {lead_id} | count={current_count + 1}",
            force=True,
        )
...
async def _handle_open(
    ...
    resolved_campaign_id = (
        campaign_id or _resolve_campaign_id(lead_id)
    )
