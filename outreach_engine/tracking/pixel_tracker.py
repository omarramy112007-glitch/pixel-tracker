...
MIN_SEND_TO_OPEN_SECONDS = 300  # 5 minutes to prevent false opens
...
def _resolve_outreach_lead(lead_id: int) -> Dict[str, Any]:
    try:
        res = (
            ...
            .select("id, email, campaign_id, open_count, followup_open_count, click_count, status, followup_status, last_email_sent")
            .eq("id", lead_id)
            .limit(1)
            .execute()
        )
...
def _update_outreach_lead_counters(lead_id: int, event_type: str, row: Dict[str, Any]) -> None:
    ...
    - Uses followup_status to choose the correct counter
    """
    try:
        now = _utc_now_iso()
            ...
            if last_email_sent:
                try:
                    ...
                    if time_since_send < MIN_SEND_TO_OPEN_SECONDS:
                        print(f"⏳ Open ignored (too soon after send: {time_since_send:.0f}s) → lead_id={lead_id}")
                        return
                except Exception as e:
