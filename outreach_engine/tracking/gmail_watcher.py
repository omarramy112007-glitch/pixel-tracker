...
GMAIL_USER_EMAIL = (
    os.getenv("GMAIL_USER_EMAIL", "")
    .strip()
    .lower()
)

DEBUG_LOGS = (
    os.getenv("GMAIL_DEBUG_LOGS", "false")
    .strip()
    .lower()
    == "true"
)

MAX_THREAD_LOOKUPS_PER_LEAD = int(
    os.getenv("GMAIL_MAX_THREAD_LOOKUPS", "5")
)

THREAD_LOOKBACK_DAYS = int(
    os.getenv("GMAIL_THREAD_LOOKBACK_DAYS", "30")
)
...
def _load_credentials_from_b64_env():
    ...
    try:
        ...
        if not isinstance(token_info, dict):
            raise ValueError(
                "Decoded GMAIL_TOKEN_B64 is not a JSON object"
            )
        ...
        log(
            "✅ Using GMAIL_TOKEN_B64 credentials",
            force=True,
        )
        ...
        log(
            f"⚠ Failed to load GMAIL_TOKEN_B64 credentials: {e}",
            force=True,
        )
...
def _load_credentials_from_file():
    ...
    try:
        ...
        log(
            f"📄 Loaded Gmail token.json from: {TOKEN_JSON_PATH}",
            force=True,
        )
...
def _coerce_credentials(raw):
    ...
    raise TypeError(
        f"Unsupported Gmail credentials type: {type(raw)!r}"
    )
...
def _save_credentials(creds) -> None:
    try:
        if hasattr(creds, "to_json"):
            ...
            log(
                f"💾 Saved Gmail token.json to: {TOKEN_JSON_PATH}",
                force=True,
            )

    except Exception as e:
        log(
            f"⚠ Failed to save token.json: {e}",
            force=True,
        )
    ...
    if (
        ...
        try:
            if GoogleAuthRequest is None:
                raise RuntimeError(
                    "Google auth request class unavailable"
                )
...
def get_service():
    ...
    creds = _ensure_credentials_valid(
        _coerce_credentials(raw)
    )
...
def _load_last_history_id() -> Optional[str]:
    try:
        ...
        log(
            f"⚠ Could not load history_id from Supabase: {e}",
            force=True,
        )
...
def _build_topic_name(
    ...
    if not project_id:
        raise ValueError(
            "GMAIL_PROJECT_ID is empty"
        )

    return f"projects/{project_id}/topics/{topic_name}"


def _is_reply_headers(
...
def _lookup_system_lead_id(
    ...
    try:
        ...
        log(
            f"⚠ System lead lookup failed for {lead_email}: {e}",
            force=True,
        )
...
def _lead_is_eligible(lead: Dict[str, Any]) -> bool:
    ...
    return bool(
        ...
        or status in {"sent", "pending", "opened"}
    )
...
def _fetch_candidate_leads(
    ...
    try:
        ...
        return [
            lead
            for lead in leads
            if _lead_is_eligible(lead)
        ]

    except Exception as e:
        log(
            f"⚠ Failed to fetch candidate leads: {e}",
            force=True,
        )
...
def _candidate_thread_ids_for_lead(
    service,
    lead: Dict[str, Any],
) -> List[str]:
    ...
    try:
        query = (
            f"from:{email} "
            f"newer_than:{THREAD_LOOKBACK_DAYS}d "
        ...
        log(
            f"⚠ Thread search failed for {email}: {e}",
            force=True,
        )
...
def _reply_already_recorded(
    ...
    try:
        ...
        for row in res.data or []:
            ...
            if (
                msg_id
                and metadata.get("gmail_message_id") == msg_id
            ):
                return True
...
def _update_reply_metrics(
    ...
    try:
        ...
        log(
            f"⚠ Failed to update outreach_leads metrics: {e}",
            force=True,
        )
    ...
    try:
        ...
        if existing.data:
            supabase.table("crm_analytics").update(
                {
                    "replies": int(
                        existing.data[0].get("replies") or 0
                    ) + 1,
        ...
        log(
            f"⚠ Failed to update crm_analytics: {e}",
            force=True,
        )
...
def _process_thread_for_lead(
    ...
    system_lead_id = _lookup_system_lead_id(
        lead_email
    )

    try:
        thread_ids = _candidate_thread_ids_for_lead(
            service,
            lead,
        )
        ...
        for thread_id in thread_ids:
            ...
            try:
                ...
                log(
                    f"⚠ Failed to load thread {thread_id}: {e}",
                    force=True,
                )

                continue

            reply = _thread_has_reply(
                thread,
                lead_email,
            )
            ...
            _mark_processed(
                msg_id or f"{outreach_id}:{thread_id}"
            )
            ...
            log(
                f"✅ Reply saved → Lead {outreach_id} | Campaign {campaign_id} | Email {lead_email}",
                force=True,
            )
        ...
        log(
            f"⚠ Failed to process lead {outreach_id}: {e}",
            force=True,
        )
...
def check_for_replies(
    ...
    if not GOOGLE_LIBS_AVAILABLE:
        log(
            "⚠ Gmail reply checking disabled: google libraries missing",
            force=True,
        )
    ...
    for lead in leads:
        try:
            result = _process_thread_for_lead(
                service,
                lead,
            )
...
async def start_reply_polling(
    ...
    log(
        f"👂 Reply polling started every {interval_seconds}s",
        force=True,
    )
    ...
    while True:
        ...
        try:
            ...
            if processed:
                log(
                    f"✅ Replies processed: {len(processed)}",
                    force=True,
                )

        except Exception as e:
            log(
                f"⚠ Reply polling error: {e}",
                force=True,
            )
...
def start_watch() -> Dict[str, Any]:
    ...
    try:
        ...
        log(
            f"❌ Gmail watch failed: {content or e}",
            force=True,
        )
        ...
        log(
            f"❌ Unexpected error starting Gmail watch: {e}",
            force=True,
        )
